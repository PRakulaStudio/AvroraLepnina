...
$MYSQL_CONNECTION = mysqli_connect(MYSQL_SERVER, MYSQL_USER, MYSQL_PASS, MYSQL_DB);
...
class DB
{
    ...
    public static function select_debug($table_name, $fields, $where = "", $order = "", $up = true, $limit = "", $offset = "")
    {
        global $MYSQL_CONNECTION;
        for ($i = 0; $i < count($fields); $i++) {
            if ((strpos($fields[$i], "(") === false) && ($fields[$i] != "*")) $fields[$i] = "`" . $fields[$i] . "`";
